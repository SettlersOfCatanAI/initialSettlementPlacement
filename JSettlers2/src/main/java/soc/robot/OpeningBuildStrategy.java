...
public class OpeningBuildStrategy {
    ...
    public int planInitialSettlements()
    {
        ...
        int[] hexWeights = {0,0,0,0,0,0}; // index corresponds to hex type, e.g. 0=water, 1=clay, etc.
        
        
        /* Calculate "weighted value" for each hex type which we define as
         * the total number of ways to roll the dice numbers on each tile with that hex type
        */
        for (int i=0; i<= 36; i++)
        {
        	// Calculates the number of ways to receive each dice roll number
        	// E.g. if numberLayout[i] == 2, hexWeights will be 1 since there is one way to 
        	// make 2 with two dice
        	int val = board.getHexLayout()[i] <= 5 ? board.getHexLayout()[i] : 0;
        	hexWeights[val] += 6-Math.abs(board.getNumberLayout()[i]-7);
        }
        
        
        
        List<Pair<Integer, Integer>> bestSettlementLocations = new ArrayList<Pair<Integer, Integer>>();

        // Here we loop through all potential settlements and score them based on
        // the dice roll and weighted value of their adjacent hexes
        for (int i = 0; i < ourPotentialSettlements.length; ++i)
        {
        	List<Integer> adjacentHexCoords = board.getAdjacentHexesToNode(ourPotentialSettlements[i]);
    		
    		List<Integer> hexDiceRolls = new ArrayList<Integer>();
    		
    		adjacentHexCoords.stream().map(board::getNumberOnHexFromCoord).forEach(hexDiceRolls::add);

    		int totalRawScore = 0;
    		
    		// for the dice roll numbers on adj hexes, calculate how far they are from 7
    		// since 6 or 8 are the ideal dice numbers
    		for (Integer diceRollNumber : hexDiceRolls)
    		{
    			totalRawScore += Math.abs(7 - diceRollNumber);
    		}

    		// similarly compute the total weighted score for the settlement
    		int totalWeightedScore = 0;
    		for (Integer coord : adjacentHexCoords)
    		{
    			int val = board.getHexTypeFromCoord(coord) <= 5 ? board.getHexTypeFromCoord(coord) : 0;
    			totalWeightedScore += hexWeights[val];
    		}
    		
    		int settlementScore = totalWeightedScore - totalRawScore;
    		
    		// pair with settlement in first spot, score in second spot (for future sorting)
    		Pair<Integer, Integer> settlementAndScore = new Pair<>(ourPotentialSettlements[i], settlementScore);
    		
        	bestSettlementLocations.add(settlementAndScore);
        	
        }
        
        // sort all settlements based on their total score
        Collections.sort(bestSettlementLocations, Comparator.comparing(p -> -p.getB()));        

        // settlement with the highest score from the list
        return bestSettlementLocations.get(0).getA();
 ...
 int[] hexWeights = {0,0,0,0,0,0}; 
        
        
        // Calculate weighted value for each hex type
        for (int i=0; i<= 36; i++)
        {
        	// Calculates the number of ways to receive each dice roll number
        	// E.g. if numberLayout[i] == 2, hexWeights will be 1 since there is one way to 
        ...
        System.out.println("Best settlement locs: ");
        String foo = "";
        for (Pair<Integer, Integer> pair : bestSettlementLocations) {
            foo += pair.getA() + ", ";
        }
